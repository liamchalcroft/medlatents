"""Training script for MaskGIT bidirectional transformer models."""
...
from medlatents.maskgit import MaskGIT  # noqa: E402
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for step, batch in progress_bar:
            ...
            with accelerator.accumulate(model):
                # Calculate dynamic mask ratio based on training progress
                progress = global_step / (args.epochs * len(train_loader))
                curr_mask_ratio = get_mask_ratio(progress, args.mask_schedule)

                # Forward pass using built-in masking
                logits = model(batch, mask_ratio=curr_mask_ratio)
                loss = F.cross_entropy(logits.reshape(-1, model.vocab_size), batch.reshape(-1))
            ...
            accelerator.log(
                {
                    ...
                    "train/lr": lr,
                    "train/mask_ratio": curr_mask_ratio,
                }
            )
...
def main():
    ...
    parser.add_argument(
        ...
        help="Schedule for mask ratio during training",
    )
